import { setJournalType } from './journal.js';

import { toggleSignClass } from './forms.js';

import { textContainer, cleanContainer } from './textCont.js';
...
function addListenerFromAction(e) {
  e.addEventListener('click', () => {
    ...
    if (action in actions) actions[action].apply(e);
  });
}
...
function saveEntry() {
  ...
  /*     else if (textContainer.querySelector("h1").innerHTML === "")
            window.alert("Entry must have a title."); */ else {
    openModal();
    const textContainerCopy = cleanContainer();
    const title = getTitle(textContainerCopy);
...
function getTitle(textContainerCopy) {
  let title = textContainer;
  if (textContainerCopy.querySelector('h1'))
    title = textContainerCopy.querySelector('h1');
  else if (textContainerCopy.querySelector('h2'))
    title = textContainerCopy.querySelector('h2');
  else if (textContainerCopy.querySelector('h3'))
    title = textContainerCopy.querySelector('h3');
  else title = textContainerCopy.querySelector('p');
  console.log(title);
  return title;
}
