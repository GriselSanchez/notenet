...
export function setJournalType() {
  ...
  //arreglar pora cuando expire el token
    ...
    if (
      journal.classList.contains('active') &&
      type === journal.getAttribute('data-type')
    ) {
      closeJournal();
    } else {
      closeJournal();
...
function getJournalEntry(j) {
  ...
  let buttons = getJournalEntryButtons(j, random_color);
  ...
  for (let i of journalElements) {
    ...
    c++;
    if (c > 5) {
      enableShowMoreButton(journalElements, journalItem, buttons, random_color);
      break;
    }
  }
  ...
  journalItem.style.backgroundColor = random_color;
}

function getRandomColor() {
  var colors = ['#f2e8ba', '#caffc9', '#b9cced', '#f4d8fe'];
  var random_color = colors[Math.floor(Math.random() * colors.length)];
  return random_color;
}

function enableShowMoreButton(journalElements, div, buttons, color) {
  ...
  showMoreButton.style.backgroundColor = color;
...
export function getJournalMainButtons() {
  ...
  if (productCount <= resPerPage * currentPage)
